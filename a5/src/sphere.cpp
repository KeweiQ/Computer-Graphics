  ...
  Eigen::MatrixXi & NF)
{
  ...
  UV.resize(num_faces_u * num_faces_v + num_faces_u + num_faces_v + 1, 2);
  ...
  for (int v = 1; v < num_faces_v; v++) {
    for (int u = 0; u < num_faces_u; u++) {
      ...
      UV.row(uv_row) << u / num_faces_u, v / num_faces_v;
      uv_row += 1;
    }
  ...
  for (int v = 0; v < num_faces_v; v++) {
    for (int u = 0; u < num_faces_u; u++) {
      int botton_left = (v - 1) * (num_faces_u + 1) + u + 1;
      int botton_right = (v - 1) * (num_faces_u + 1) + u + 2;
      int top_left = v * (num_faces_u + 1) + u + 1;
      int top_right = v * (num_faces_u + 1) + u + 2;
      ...
      F.row(f_row) << botton_left, botton_right, top_right, top_left;
      f_row += 1;
